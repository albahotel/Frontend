<template>
  ...
  <div class="flex-1 min-w-0 flex justify-center">
    <div class="flex items-center gap-2 truncate"> <!-- truncate для длинного текста -->
      <svg
      ...
      <h6 class="text-xl leading-8 font-semibold text-gray-900 truncate"> <!-- truncate для текста -->
        {{ weekRange }}
      </h6>
    </div>
  </div>

  <!-- Правая кнопка с фиксированной позицией -->
  <button
    @click="$emit('change-week', 1)"
...
const emit = defineEmits(['new-booking', 'change-week'])
...
onMounted(async () => {
  generateWeekDays() // Добавляем генерацию дней
