    ...
    <section class="relative bg-stone-50 py-24">
      <div class="w-full max-w-7xl mx-auto px-6 lg:px-8 overflow-x-auto">
        <div class="flex flex-col md:flex-row max-md:gap-3 items-center justify-between mb-5">
          ...
          <h6 class="text-xl leading-8 font-semibold text-gray-900">{{ weekRange }}</h6>
        </div>
        <button 
      ...
      <div class="relative">
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr>
              <th class="sticky left-0 bg-stone-50 border-t border-gray-200 p-3.5 text-sm font-medium text-gray-900 text-left z-10">
                Room
              </th>
              <th 
          ...
          <tbody>
            <tr v-for="room in rooms" :key="room.id">
              <td class="sticky left-0 bg-stone-50 border-t border-r border-gray-200 p-3.5 font-medium text-gray-900 z-10">
                №{{ room.number }}
              </td>
              <td 
                v-for="day in weekDays" 
              ...
              >
                <div 
                  ...
                  :key="booking.id"
                  class="rounded p-1.5 border-l-2 border-green-600 bg-green-50 mb-1"
                >
                  <div class="text-xs font-normal text-gray-900">
                    {{ booking.customer }}
                  </div>
                  <div class="text-xs font-semibold text-green-600 space-y-1">
                    <div v-if="isCheckIn(booking, day.dateObj)" class="flex items-center gap-1">
                      <span class="text-[10px]">↑</span> {{ formatDate(booking.date_in) }}
                    </div>
                    <div v-if="isCheckOut(booking, day.dateObj)" class="flex items-center gap-1">
...
// Генерация недели с 19 по 25 мая 2025
...
// Проверка на сегодняшний день (можно убрать если не нужно)
const isToday = (date) => {
...
// Обработчик новой брони
const handleNewBooking = () => {
  // Заглушка - можно реализовать модальное окно
...
const isCheckIn = (booking, date) => {
  return booking.date_in === date.toISOString().split('T')[0]
}

// Проверка на выезд
const isCheckOut = (booking, date) => {
  return booking.date_out === date.toISOString().split('T')[0]
}
...
<style>
/* Фиксированная ширина для первой колонки */
.sticky.left-0 {
  width: 100px;
  min-width: 100px;
  max-width: 100px;
